...
export default function Home() {
  return (
    <>
      <PrismaHero />
      <footer
        className="relative z-30 -mt-28 px-8 py-8 md:-mt-32"
        style={{ background: "transparent" }}
      >
        <div className="mx-auto grid max-w-6xl grid-cols-1 items-center gap-5 md:grid-cols-3">
          <div className="flex items-center justify-center gap-3 md:justify-start">
            <Image src="/icon.png" alt="Scrollio" width={28} height={28} className="rounded-full" />
            <span className="text-xs text-white/85">
              © 2026 · Turn scrolling into learning
            </span>
          </div>
          ...
          <div className="flex items-center justify-center gap-6">
            {["Privacy", "Terms", "Contact"].map((l) => (
              <a
                key={l}
                href="#"
                className="text-xs text-white/75 no-underline transition-opacity hover:text-white"
              >
                {l}
              </a>
            ))}
